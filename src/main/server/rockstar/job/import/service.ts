...
const SUPPORTED_JOB_TYPES: JobType[] = [ RACE ];
const SUPPORTED_RACE_TYPES: RaceType[] = [ LAND, STUNT ];
...
function validateJobType(json: any) {
  ...
  } else if (!SUPPORTED_JOB_TYPES.includes(jobType)) {
...
function validateRaceType(json: any) {
  ...
  } else if (!SUPPORTED_RACE_TYPES.includes(raceType)) {
